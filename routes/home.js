...
router.get("/", async (req, res) => {
  try {
    ...
    params.joins = [
      {
        type: "INNER",
        join: ["products", "home_products.product_id", "=", "products.id"],
      },
      {
        type: "INNER",
        join: [
    ...
    params.where = [["products_images.is_first", "=", 1]];

    // establezco el limite de filas a mostrar
    params.limit = 4;

    // consulto los productos
    let products = await queryBuilder("home_products", params).catch((err) => {
      throw err;
    });

...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      data.where = [["products_images.is_first", "=", 1]];

      // realizo la consulta
      const results = await queryBuilder("products", data);

      let num_items = await querySync(
        "select count(name) as quantity from products",
        null
      );
  ...
  async (req, res) => {
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        sql =
          "INSERT INTO products_images (product_id, image, is_first) VALUES (?,?,?)";
        await querySync(sql, [result.insertId, image, i == 0]).catch((e) => {
          throw e;
        });
      }
